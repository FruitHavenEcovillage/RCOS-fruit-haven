# Proposal: Federation Sovereignty Doctrine

- **Status:** Passed
- **Submitted:** 2019-05-17
- **Submitted by:** Fruit Haven Owners
- **Decision type:** Constitutional
- **Authorized decision path:** Consensus

---

## Summary

Establishes the sovereignty and federation rules for Fruit Haven community areas. It outlines community autonomy, access rules between communities, and the formation of new sovereign communities.

## Affected Layers and Artifacts

- Layer 2 — `layers/2-governance/04-federation-protocol.md`
- Layer 2 — `layers/2-governance/02-governance-protocol.md`

## Change Type

- [x] Permanent rule change
- [ ] Time-bounded experiment

## Rationale

To explicitly define the relationships between different Fruit Haven communities, properties, and the rules governing self-governance and access across the federation.

## Risks and Mitigations

Risk: Communities may fracture or establish conflicting rules.
Mitigation: The charter establishes clear supremacy guidelines, maintaining that the core charter takes precedence where community bylaws conflict.

## Transition and Migration Plan

No migration required.

## Rollback Plan

Reversion to prior unstructured property relations through a new consensus vote.

## Effective Date

2019-05-17

## Review Date (if experiment or time-bounded)

N/A

---

## Decision Record

- **Vote outcome:** Passed
- **Vote date:** 2019-05-17
- **Mechanism:** Consensus
- **Snapshot link:** N/A
- **Signatories / vote count:** Consensus achieved
